...
function Progress({ item }: { item: BillingCase }) {
  const nodes = [
    ...
    { key: 'delivery_note', done: Boolean(item.delivery_note_complete) || stepCompleted(item.steps.delivery_note), title: 'BL' },
...
function CaseDetail({ item, interventionOptions, duplicateCases, canResolveDuplicates, canDeleteCase, history, historyLoading, onStep, onDocuments, onPayment, onHistory, onToggleBlocked, onLinkIntervention, onResolveDuplicate, onReassess, onDeleteCase }: {
  ...
  return <div className="max-h-[78vh] space-y-5 overflow-y-auto pr-1">
    ...
    <section><h3 className="mb-3 flex items-center gap-2 text-sm font-bold"><Calendar className="h-4 w-4 text-savia-accent" /> Chronologie du dossier</h3><div className="space-y-2">
      ...
      {!coverageLocked && item.contract_id && item.delivery_note_complete && <div className="rounded-xl border border-green-500/30 bg-green-500/5 p-3"><div className="flex items-center gap-3"><div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-green-500/15 text-green-300"><Check className="h-4 w-4" /></div><div><div className="font-semibold text-green-200">Bon de livraison validé automatiquement</div><p className="mt-0.5 text-xs text-savia-text-muted">Dossier global de maintenance sous contrat : la clôture de la dernière intervention permet de passer directement à la facturation.</p></div></div></div>}
      {!coverageLocked && !(item.contract_id && item.delivery_note_complete) && <DocumentCollectionRow title="Bons de livraison" documents={item.delivery_notes || []} totalKey="is_total_delivery" complete={Boolean(item.delivery_note_complete)} onOpen={() => onDocuments(item, 'delivery')} />}
