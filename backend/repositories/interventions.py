...
def supprimer_planning(planning_id):
    """Supprime une planification manuelle et son intervention liée.

    The planning generated by a maintenance contract is owned by that contract
    and must be changed from the contract workflow instead of being deleted
    independently from the planning screen.
    """
    with get_db() as conn:
        planning = conn.execute(
            """SELECT id, machine, contrat_id, recurrence, is_ghost,
                      original_planning_id
               FROM planning_maintenance WHERE id=%s""",
            (planning_id,),
        ).fetchone()
        if not planning:
            raise KeyError(f"Planning #{planning_id} introuvable")
        ...
        if planning.get("is_ghost") and planning.get("original_planning_id"):
            planning = conn.execute(
                """SELECT id, machine, contrat_id, recurrence, is_ghost,
                          original_planning_id
                   FROM planning_maintenance WHERE id=%s""",
                (planning["original_planning_id"],),
            ).fetchone()
            if not planning:
                raise KeyError(f"Planning source #{planning_id} introuvable")
            planning = dict(planning)

        if planning.get("contrat_id"):
        ...
        intervention_rows = conn.execute(
            "SELECT id FROM interventions WHERE planning_id = ANY(%s)",
            (planning_ids,),
        ).fetchall()
        ...
        # A reschedule creates an audit-only ghost linked to the source row.
        # Remove it as part of the same transaction so no orphan remains.
        conn.execute(
            "DELETE FROM planning_maintenance WHERE original_planning_id = ANY(%s)",
            (planning_ids,),
        )
        conn.execute("DELETE FROM planning_maintenance WHERE id = ANY(%s)", (planning_ids,))
    _trigger_backup()
    return {
        "deleted_planning": len(planning_ids),
        "deleted_interventions": len(intervention_ids),
        "recurring": False,
    }
