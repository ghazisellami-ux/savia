...
def update_intervention(intervention_id: int, request: Request, body: dict = Body(...), user: dict = Depends(_verify_token)):
    ...
    if user.get("role") == "Technicien":
        with get_db() as conn:
            row = conn.execute(
                """SELECT i.technicien, i.technicien_id, i.statut,
                          EXISTS(
            ...
            tech_rows = conn.execute(
                """SELECT it.technicien_id, it.technicien_nom,
                          EXISTS(
            ...
            if not is_assigned and technician_id is not None and row.get("technicien_id") == technician_id:
                is_assigned = True
                  ...
                  (row.get("technicien_id") is None or not row.get("technician_id_is_valid")) and
                  current_tech):
